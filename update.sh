...
for version in "${versions[@]}"; do
	...
	if [ $majorVersion -ge 6 ]; then
        ...
        digest="$(curl --head -fsSL -H 'Accept: application/vnd.docker.distribution.manifest.v2+json' -H "Authorization: Bearer $authToken" "https://docker.elastic.co/v2/elasticsearch/elasticsearch/manifests/$plainVersion" | gawk -F ':[[:space:]]+' '$1 == "Docker-Content-Digest" { print $2 }')"
